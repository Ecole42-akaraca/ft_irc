...
void Client::join(Channel *channel) {
	...
	snprintf(message, sizeof(message), "%s has joined channel %s.", _nickname.c_str(), channel->getName().c_str());
	ft_log(message);
}
...
void Client::leave() {
	...
	snprintf(message, sizeof(message), "%s has left channel %s.", _nickname.c_str(), name.c_str());
	ft_log(message);
}
