...
void Channel::removeClient(Client *client) {
	...
	if (_admin == client) {
		...
		// char message[256];
		snprintf(message, sizeof(message), "%s is now admin of channel %s.", _admin->getNickname().c_str(), _name.c_str());
		ft_log(message);
	}
}
...
void Channel::kick(Client *client, Client *target, const std::string &reason) {
	...
	snprintf(message, sizeof(message), "%s kicked %s from channel %s.", client->getNickname().c_str(), target->getNickname().c_str(), _name.c_str());
	ft_log(message);
}
